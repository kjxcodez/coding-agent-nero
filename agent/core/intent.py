...
_RULES: List[Tuple[re.Pattern, Intent]] = [
    ...
    (
        re.compile(r"^/?(architecture overview|component graph|show architecture|architecture map)\s*$", re.I),
        Intent.ARCHITECTURE,
    ),
    ...
    (
        re.compile(
            r"^/?(routes|show routes|list routes|list api|endpoints|api endpoints|show api|show endpoints)\s*$", re.I
        ),
        Intent.ROUTES,
    ),
    ...
    # --- Verification / test -------------------------------------------
    (
        re.compile(r"\b(run|execute) (the )?(tests?|specs?|unit tests?|integration tests?|test suite)\b", re.I),
        Intent.VERIFY,
    ),
    (
        re.compile(
            r"\b(run|start|execute|launch|boot) (the )?(server|app|application|service|script|command|binary|executable)\b",
            re.I,
        ),
        Intent.VERIFY,
    ),
    ...
    # --- Symbol lookup -------------------------------------------------
    (
        re.compile(r"\bwhere (is|are|was|can i find)\b.{1,60}\b(defined|declared|implemented|located)\b", re.I),
        Intent.SYMBOLS,
    ),
    ...
    (
        re.compile(
            r"\b(search|grep|find all|look for|scan for|list all)\b.*(uses?|calls?|references?|occurrences?|instances?)\b",
            re.I,
        ),
        Intent.SEARCH,
    ),
    ...
    # --- Explanation ---------------------------------------------------
    (
        re.compile(r"\b(explain|describe|walk me through|walk through|break down)\b.*(how|what|why)\b", re.I),
        Intent.EXPLAIN,
    ),
    ...
    (
        re.compile(
            r"\b(improve|enhance|optimize)\b.*(application|codebase|feature|logic|performance|function|class|method|module|component|route|endpoint|schema|model|code)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(
            r"\b(add|implement|create|build|write|generate|scaffold)\b.*(feature|endpoint|route|function|class|method|module|component|page|api|service|model|schema|test|migration)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(
            r"\b(fix|debug|resolve|patch|repair|correct)\b.*(bug|error|issue|problem|crash|exception|warning|failure)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(
            r"\b(refactor|restructure|reorganize|rewrite|migrate|update|upgrade|change|rename|move|delete|remove)\b.{1,60}\b(file|function|class|method|module|component|route|endpoint|schema|model|code|logic)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(
            r"\b(add|update|change|modify|edit|replace|remove)\b.{1,40}\b(field|property|attribute|column|parameter|argument|variable|constant|import|dependency)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(
            r"\b(make|set|enable|disable|configure|set up|turn on|turn off)\b.{1,60}\b(work|works|working|function|functions|available|optional|required|default)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
    (
        re.compile(r"\bplease\b.{0,30}\b(add|create|fix|implement|update|change|modify|refactor|build|write)\b", re.I),
        Intent.MODIFY,
    ),
    # --- General modification fallback ---------------------------------
    (
        re.compile(
            r"^(add|create|implement|fix|build|write|generate|update|change|modify|refactor|delete|remove|rename|improve|enhance)\b",
            re.I,
        ),
        Intent.MODIFY,
    ),
